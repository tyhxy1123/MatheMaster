...
\begin{proof}
	...
	%TODO change \not\subset to \not proper subset in the following few cases
	...
	Sei also $\ineq(P)\neq \emptyset$.
	 \begin{enumerate}[label = (\alph*)]
		\item \glqq $\leftarrow$\grqq\ Sei $F$ eine Seitenfläche mit $\dim F = \dim P -1$, so ist $F$ eine echte Seitenfläche von $P$.
			...
			%TODO why does lightning not work?
...
\begin{satz}
	...
	\end{enumerate}
\end{satz}
\subsectin{Alternativer Zugang: Polyeder \glqq von innen\grqq\ beschreiben}
...
\begin{lemma}
	(\underline{Farkas})\\
	%TODO add pictures IMPORTANT 
	%TODO change ° to something that work that works
