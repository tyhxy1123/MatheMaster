...
\begin{align*}
	...
	\quad,(x,s)& \geq 0
\end{align*} 
Gilt in einer Lösung ($x^0$), dass $s^* =0$, so ist das $x^*$ zulässig für \eqref{nf} und die Basis des Hilfsproblems \glqq ist\grqq\ eine primal zulässige Basis für \eqref{nf}. 
...
\begin{beispiel}
	...
	\begin{enumerate}
		\item Für jeden Stab $i$ beschreiben $x_{i1},x_{i2},x_{i3} \in \N_{0}$. Wieviele Stäbe der Länge $25,30,35$ cm daraus geschnitten werden sollen.
			...
			% use label instead of referencing it directly
			\begin{align*}
		...
		\begin{align*}
			\text{mindestens } &80 \text{ Stäbe mit }25\text{cm}& 4x_{1} + 2x_{2} + 2x_{3} + x_{4} + x_{5} + x_{6} &\geq 80\\
							   &75\text{ Stäbe mit } 30 \text{cm}& x_{2} + 2x_{4} + x_{5} + 3x_{7} + 2x_{8} + x_{9} &\geq 75\\
							   &105\text{ Stäbe mit } 35 \text{cm}& x_{3} + x_{5} + 2x_{6} + x_{8} + 2x_{9} &\geq 105
    	\end{align*}
