...
\begin{theorem}
	...
	\begin{equation*}
    H_{\text{opt}}=\underbrace{ \set{1,..,m }}_{\text{ganzes $A$ in Basismatrix}}\cup \, (N+\underbrace{m}_{\text{offset}})
	\end{equation*}
...
\begin{enumerate}%[label = Schritt \arabic)]
    ...
    Gilt $\alpha_{N} \leq 0 $: \textbf{Stop}, \eqref{dualLP} ist unbeschränkt ($\to$ \eqref{nf} unzulässig).\\
		Andernfalls berechne $\gamma = \min \set{ \frac{z_{i}}{\alpha_{i}} \mid j \in N , \alpha_{j} >0 }$ und wähle $j \in N$ mit $\gamma = \frac{z_{j}}{\alpha_{j}}$
	\item Update:
		\begin{align*}
