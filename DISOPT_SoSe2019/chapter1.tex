...
(primales) LP 
\begin{equation*	}
	\min_{x}c^{T} x \text{ subject to }Ax = b, x \geq 0 (P)
\end{equation*	}
duales LP 
\begin{equation*}
	\max_{x}b^{T} y \text{ subject to }A^{T} y \leq c, x \geq 0 (P)
\end{equation*}
Annahme $A \in \R^{m \times n}$ hat Rang $m$ \\
...
$\to$ duale Normalform: 
\begin{equation*}
	\max_{y,z} b^{T} y \text{ subject to }(A^{T}  I) (y z)^{T} = c, \quad z \geq 0, \quad y \in \R^m
\end{equation*}
Basis von (D'):$ ( \underbrace{A^{T}}_{m} \underbrace{I}_{n}) \in R^{n \times(m +n)}$
...
\begin{enumerate}%[label = Schritt \arabic)]
	...
	\item Update:
		\begin{align*}
			...
			z_{i}^+ &= \gamma\\
			y^+ &= y -\gamma w\\
			N^+ &= (N \textbackslash \left\{ j \right\}) \cup \left\{ i \right\}\\
			B^+ &= (B \textbackslash \left\{ i \right\}) \cup \left\{ j \right\}
		\end{align*}
\end{enumerate}
...
\begin{align*}
	A_{B}x_{B} + A_{N_{l}}x_{N_{l}} + A_{N_{u}} x_{N_{u}} = b \implies x_{B} = A_{B}^{-1} (b - A_{N_{l}} x_{N_{l}} - A_{N_{l}}x_{N_{u}})\\
