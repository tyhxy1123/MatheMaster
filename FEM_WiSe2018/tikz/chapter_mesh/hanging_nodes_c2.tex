\begin{figure}[H]
	...
	\draw (AD) --(BC);
	\draw (M) --(CD) -- (BC);
	\foreach \i in {A,B,C,D,M,AD,CD,BC,MMC}{
	...
	\draw (M1) --(CD1) -- (BC1);
	\draw (MMC1) --(MMBC1);
	\foreach \i in {A1,B1,C1,D1,M1,AD1,CD1,BC1,MMC1,MMBC1}{
		\filldraw (\i) circle (1.5pt);
	}
	...
	\draw (M2) --(CD2) -- (BC2);
	\draw (MMC2) --(MMBC2);
	\foreach \i in {A2,B2,C2,D2,M2,AD2,CD2,BC2,MMC2,MMBC2,MMB2}{
		\filldraw (\i) circle (1.5pt);
	}
	...
	\draw (BC3) --(MMB3);
	\draw (M3) --(AB3);
	\foreach \i in {A3,B3,C3,D3,M3,AD3,CD3,BC3,MMC3,MMBC3,MMB3,AB3}{
		\filldraw (\i) circle (1.5pt);
	}
...
%	\draw[-to] (8/5*\h,-2.5*\h) -- ++(\h/2,0);
	...
	% sixth rectangle
	\coordinate (A5) at (\xone  ,\yone -3*\h);
