...
\item \textbf{classical solution} of (\ref{poisson}), then it is also a weak solution of \eqref{weak_form}. 
\end{enumerate}
...
\begin{enumerate}[=(\alph*)]
	...
	with 
	\begin{equation*}
		Lu = - \displaystyle \sum^d_{i,j} \left(  a_{ij}(x)  u_{x_j} \right)_{x_i} + c(x)u.
	\end{equation*}
	\begin{example}
		\begin{align*}
			d &= 1,&   a_{11}(x)&=1,              & c(x)&=0 \\
	...
	weak form 
	\begin{align*}
		a(u,v) &= \displaystyle \sum_{i,j} \int \limits_\Omega a_{ij}(x) u_{x_i} v_{x_j} \diff x + \int \limits_\Omega cuv \diff x \\
	...
	\begin{equation*}
		G(v) = F(v) - \displaystyle \sum^d_{i,j} \int \limits_\Omega a_{ij}\; g_{x_j} v_{x_i} \diff x - \int \limits_\Omega cgv \diff x \quad \forall v\in H^1_0(\Omega).
	\end{equation*}
	...
	\begin{thrm}
		If $\Omega \subset \R^d\ (d\geq1)$ bounded, $a_{ij},c \in L^\infty(\Omega), c\geq 0, f\in L^2(\Omega), g\in H^1(\Omega),\ L$ is elliptic, then there exists a unique solution $u \in H^1(\Omega)$ of $Lu=f$ with  $u-g \in H^1_0(\Omega)$ and 
		\begin{equation*}
	...
	\begin{proof_}
		...
		\begin{itemize}
			...
			\begin{align*}
				|a(u,v)| &\leq \displaystyle \sum^d_{i,j=1} \int \limits_\Omega |a_{ij}| |u_{x_i}| |v_{x_j}| \diff x + \int \limits_\Omega |c||u||v| \diff x \\
				& \leq \underset{i,j = 1,\dots,d}{\max} \Big\{ \|a_{ij}\|_{L^\infty}, \|c\|_{L^\infty} \Big\}\left( \displaystyle \sum^d_{i,j=1} \|u_{x_j}\| \|v_{x_i}\| + \|u\|\|v\| \right)\\
			...
			\begin{align*}
				a(u,u) &\geq \lambda \displaystyle\sum^d_{i=1} \int \limits_\Omega |u_{x_i}|^2 \diff x\\
					   &= \lambda\|\nabla u\|_{L^2}\\
			...
			\begin{align*}
				|G(v)| & \leq \|f\|_{L^2}\|v\|_{L^2} + \underset{i,j=1, \dots, d}{\max} \|a_{ij}\|_{L^\infty} \|\nabla v\|_{L^2} \|\nabla g\|_{L^2} + \|c\|_{L^\infty}\|g\|_{L^2}\|v\|_{L^2}\\
					   & \leq \big (  \|f\|_{L^2} + C_i\|g\|_{H^1} \big ) \|v\|_{H^1}\\ 
			\end{align*}
...
\begin{definition_}
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item $\Sigma_{\tau}$ linearly independent, continuous functionals $B_1,\dots,B_n:C^s(\tau) \to \R$, $s \in \N_0$ with $\forall \alpha_1, \dots, \alpha_n \in \R$ exists $p \in P_{\tau}$ s.t. $B_i (P) = \alpha_i$, $i = 1, \dots,n$
	\end{enumerate}
\end{definition_}
...
\begin{align*}
	...
	B &= (a_2-a_1, a_3-a_1)	,\quad a \in \hat{\tau}
\end{align*}
$\implies$
\begin{equation*}
	|\det B| = (a_2-a_1)(a_3-a_1)\sin \Theta
\end{equation*}
because of the following equation. Let
\begin{equation*}
	B(u,v) = 
	\begin{pmatrix}
	u_1 & v_1\\
