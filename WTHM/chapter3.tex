...
\begin{defi}
Sei $(\F_n)_{t\in I}$ eine Filtration und $\tau$ eine Stoppzeit bgzl. dieser. Wir definieren 
\begin{align*}
...
\item $\begin{aligned}
\E[X_\sigma]=\E[X_\tau~|~\F_\sigma]\qquad\forall
...
Es gilt:
\begin{align*}
\lbrace\rho\leq\tau\rbrace=\big(\underbrace{\lbrace\sigma\leq k\rbrace\cap \overbrace{A}^{\in\F_\sigma}}_{\in\F_k}\big)\cup\big(\underbrace{\lbrace\rho\leq k\rbrace\cap \overbrace{A^C}^{\in\F_\tau}}_{\in\F_k}\big)\in\F_k\qquad\forall k\in\N_0
\end{align*}
...
&\implies
\E[X_\sigma\cdot\indi_A]
=\E[X_\tau\cdot\indi_A]\qquad\forall A\in\F_\sigma\\
&\implies
X=\E[X_\tau~|~\F_\sigma]
\end{align*}
\end{proof}
