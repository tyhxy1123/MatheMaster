...
Wahrscheinlichkeit eines Ereignisses $A\in\A$ & $\P(A~|~\F)$ & --- & $\P(A~|~B)$ \\\hline
Erwartungswert einer Zufallsvariable $X$ & $\E[X~|~\F]$ & $\E[X~|~Y]~~~(\ast)$ & $\E[X~|~B]$
\end{tabular}\\
...
&=\frac{x\cdot\exp(-\lambda\cdot x)}{-\lambda}\Big|_{x=a}^\infty+\int\limits_a^\infty\frac{\exp(-\lambda\cdot x)}{\lambda}\d x=\frac{a\cdot\exp(-\lambda\cdot a)}{\lambda}+\frac{\exp(-\lambda\cdot a)}{\lambda^2}\\
&\implies
