...
&$\sigma$-Algebra $\F$ & Zufallsvariable $Y$ & Ereignis $B$\\ \hline
	$\begin{array}{c}\text{Wahrscheinlichkeit}\\\text{eines Ereignisses}\\A\in\A \end{array}$ & $\P(A~|~\F)$ & --- & $\P(A|B)=\frac{\P(A\cap B)}{\P(B)}$ \\\hline
$\begin{array}{c} \text{Erwartungswert}\\ \text{einer Zufallsvariable } X\end{array} & $\E[X~|~\F]$ siehe 1.1 und 1.2 & $\E[X~|~Y]~~~(\ast)$ & $\E[X|B]=\frac{\E[X\indi_B]}{\P(B)}$
\end{tabular}\nl
