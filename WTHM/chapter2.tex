...
\begin{beispiel} %NoNumber
(Siehe Beispiel des Kapitels.) Sei $S_n:=\xi_1+\ldots+\xi_n$ Random Walk mit $\E[\xi_i]=0$ und $\Var(\xi_i)=\sigma^2$. Dann ist
...
\begin{align*}
(\vartheta\cdot M)_n:=\sum\limits_{i=1}^n\vartheta_i\cdot(M_i-M_{i-1},\qquad(\vartheta\cdot M)_0:=0
\end{align*} 
...
\item $(\vartheta\cdot M)_n$ ist der Gesamtgewinn nach der $n$-ten Runde mit Einsatz $\vartheta_n)_{n\in\N}$
\end{itemize}
...
\end{itemize}
Deswegen heißt $(\vartheta\cdot M)_n$ auch \textbf{diskretes stochastisches Integral}
\end{itemize}
\end{bemerkung}
...
\begin{align*}
:\Longleftrightarrow\exists(K_n)_{n\in\N}\subseteq\R:\forall k\in\lbrace1,\ldots,n\rbrace:|\vartheta_k|\leq K_n TODO
\end{align*}
\end{defi}
...
\begin{enumerate}[label=(\alph*)]
\item ToDo
%\item $M\mapsto(\vartheta\cdot M)$ und $\vartheta\mapsto(\vartheta\cdot M)$ sind lineare Abbildungen
%\item $M$ Martingal und $\vartheta$ lokal beschränkt $\implies(\vartheta\cdot M)$ ist Martingal
%\item $M$ Sub-/Supermartingal und $\vartheta\geq0$ lokal beschränkt $\implies(\vartheta\cdot M)$ ist Sub-/Supermartingal
%\item $M\subseteq L_2,~M$ Martingal, $\vartheta\subseteq l_2\implies(\vartheta\cdot M)$ ist Martingal
%\item $M\subseteq L_2,~M$ Sub-/Supermartingal und $\vartheta\subseteq L_2\mit\vartheta\geq0\implies(\vartheta\cdot M)$ ist Sub-/Supermartingal
\end{enumerate}
\end{theorem}
\begin{proof}
\underline{Zeige (a):} Ist trivial.\\
...
\item Martingaleigenschaft:
\begin{align*}
\E\big[(\vartheta\cdot M)_n-(\vartheta\cdot M)_{n-1}~\big|~\F_{n-1}\big]
...
\begin{proof}
\underline{Zeige (a):}\\
Aus Theorem \ref{theorem2.5} folgt Martingaleigenschaft von $(\vartheta\cdot M)_n$:
\begin{align*}
\E\big[(\vartheta M_k)^2\big]\leq K_n^2\cdot\E\big[M_k^2\big]<\infty\qquad\forall j,k\leq n\\
\implies(\vartheta\cdot M)\in M_2
\end{align*}
\underline{Zeige (b):}
\begin{align*}
...
\end{align*}
%ToDo: better \cdot for difference between both notations
