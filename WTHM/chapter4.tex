...
&\leq \E\Big[|Y|\cdot\indi_{\lbrace |X_i|\geq R\rbrace}\Big]
...
\implies
\rho(R)
&\leq\E\Big[|Y|\cdot\indi_{\lbrace |Y|\geq R\rbrace}\Big]\stackrel{R\to\infty}{\longrightarrow}0
\end{align*}
Dies folgt aus Lemma \ref{lemma4.3} (a).
\end{proof}
...
Sei $(X_n)_{n\in\N}$ ein $(\F_n)_{n\in\N}$-Martingal. Dann sind äquivalent:
\begin{enumerate}[label=(\alph*)]
\item $\begin{aligned}
