...
\end{align*}
Insbesondere gilt $X_\infty(-\infty,\infty)$.
\end{proof}
...
%TODO Hier Skizze einfügen für d=2 und L=4
Jedes Individuum hat (Schwarz-Weiß-)Meinung in $\lbrace0,1\rbrace$, also dafür oder dagegen. Zustand des Kollektivs: $x\in\lbrace0,1\rbrace^{\Lambda}=\big\lbrace f:\Lambda\to\lbrace 0,1\rbrace\big\rbrace=\lbrace0,1\rbrace^{L^d}$\\
...
\begin{itemize}
\item $I_n$ gleichverteilt auf dem Gitter $\Lambda$ und $N$
\item $N_n$ gleichverteilt auf $(\pm e_i)_{i\in\lbrace1,\ldots,d\rbrace}$
\end{itemize}
...
\end{align*}
\ul{Fragen:} Langzeitverhalten? Tritt Konsens ein?
\\
...
\end{align*}
\end{itemize}
Folglich ist $(M_n)$ ein beschränktes Martingal und somit existiert gemäß Theorem \ref{theorem4.2Martingalkonvergenz} $M_\infty\mit\limn M_n=M_\infty$ (und in $L_1\implies\E[M_\infty]=\E[M_0]$).\\
...
\begin{align}
\P(X_n\neq X_{n-1}~|~X_{n-1}=x)\nonumber
&\geq \P\big(I_n=i\wedge I_n+N_n=j~(\text{mod } L)\big)\\
&=\P\big(\underbrace{I_n=i\wedge N_n=i-j}_{\unab}~(\text{mod  }L)\big)\nonumber\\
...
\begin{align*}
\langle M\rangle_n 
&=\sum\limits_{k=1}^n\E\left(\left.\underbrace{(M_m-M_{k-1})}_{\in\lbrace0,1\rbrace}^2~\right|~\F_{k-1}\right]\\
...
&\implies M_n(\omega)\in\lbrace0,\Lambda^d\rbrace\qquad\forall N_0(\omega)\\
