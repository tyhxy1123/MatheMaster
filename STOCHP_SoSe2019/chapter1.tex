...
\begin{bemerkungnr}\label{bemerkung1.3.2}\
	\begin{enumerate}[label=(\arabic*)]
		...
		\item Ein Maßraum $(\Omega,\A,\P)$ heißt \define{vollständig}, falls $\A$ jede $\P$-Nullmenge enthält.\label{item:bemerkung1.3.2(7)}
...
\begin{aufgabenr}[1.3.2.7 Teil 1, \texorpdfstring{\hyperref[loes:3]{Lösung siehe Anhang}}{Lösung siehe Anhang}]\label{aufg:3}\enter
	Zeigen Sie:
	\begin{enumerate}
...
\begin{aufgabenr}[1.3.4, \texorpdfstring{\hyperref[loes:5]{Lösung siehe Anhang}}{Lösung siehe Anhang}]\label{aufg:5}\enter
...
\begin{bemerkung}%\label{bem1.3.8Kolmo}
...
\begin{aufgabenr}[1.3.8, \texorpdfstring{\hyperref[loes:6]{Lösung siehe Anhang}}{Lösung siehe Anhang}]\label{aufg:6}\enter
...
\begin{aufgabenr}[1.3.16, \texorpdfstring{\hyperref[loes:9]{Lösung siehe Anhang}}{Lösung siehe Anhang}]\label{aufg:9}\
	\begin{enumerate}[label=(\arabic*)]
		...
		Zeigen Sie, dass\label{item:aufg9(1)}
		\begin{align*}
		...
		\item Welche Eigenschaft muss die Funktion $g\colon \R^d\to\C$ haben, damit \label{item:aufg9(2)}
		\begin{align*}
...
\begin{aufgabenr}[1.3.17, \texorpdfstring{\hyperref[loes:10]{Lösung siehe Anhang}}{Lösung siehe Anhang}]\label{aufg:10}\enter
	Sei $\alpha\in\R$ und 
	\begin{align*}
	...
	\begin{enumerate}[label=(\arabic*)]
		\item Zeigen Sie, dass $X$ stationär ist und berechnen Sie Korrelationsfunktion.
		\label{item:Aufg:10(1)}
		\item Ist $X$ stationär im engeren Sinne?
		\label{item:Aufg:10(2)}
	\end{enumerate}
\end{aufgabenr}
...
\begin{bemerkung}\
	\begin{enumerate}
		\item "Zukunft hängt nur von der Gegenwart ab, nicht von der Vergangenheit"
		\item $X$ ist Markovscher Prozess auf $T$
		\begin{align*}
			\gdw\P\Big(X_{t_n}\leq a_n~\Big|~\forall j\in\set{1,\ldots,n-1}:X_{t_j}=a_j\Big)
			&=\P\Big(X_{t_n}=a_n~\Big|~X_{t_{n-1}}=a_{n-1}\Big)
		\end{align*}
		für alle $n\in\N$, für $\mu_{(X_1,\ldots,X_n)}$-fast alle $(a_1,\ldots,a_n)\in\R^n$, $t_j\in T$ mit $t_1<\ldots<t_n$.\\
		Mit anderen Worten:
		Wenn die Vergangenheit $X_{t_1},\ldots,X_{t_{n-2}}$ und die Gegenwart $X_{t_{n-1}}$ gegeben sind, dann hängt die Zukunft $X_{t_n}$ nur von der Gegenwart ab.
		%Später werden wir eine allgemeinere Definition geben.
		\item Die Bedingungen können Nullereignisse sein. %Dr. Kühn
	\end{enumerate}
\end{bemerkung}
...
\begin{beisp}\
	\begin{itemize}
		...
		\item Physikalische Prozesse:
		\begin{itemize}
			\item Beim radioaktiven Zerfall die Anzahl der zur Zeit $t$ noch vorhandenen radioaktiven Atome.
			\item Bei der Brownschen Bewegung die Position des Partikels.
		\end{itemize}
	\end{itemize}
\end{beisp}

\begin{aufgabenr}[1.3.19, \texorpdfstring{\hyperref[loes:11]{Lösung siehe Anhang}}{Lösung siehe Anhang}]\label{aufg:11}\enter
	Wir betrachten den Wahrscheinlichkeitsraum
	\begin{align*}
		(\Omega,\A,\P)=\Big([0,1],\B\big([0,1]\big),\lambda|_{[0,1]}\Big)
	\end{align*}
	und definieren den Prozess $X$ durch
	\begin{align*}
		X_t(\omega)&:=t\mal\omega\qquad\forall t\in(0,\infty)=:T,\forall \omega\in\Omega
	\end{align*}
	Bestimmen Sie die endlich-dimensionale Verteilung des Prozesses $X$ und die Kovarianzfunktion. (Die Pfade sind Geraden $\leadsto$ stetig)
\end{aufgabenr}

\begin{aufgabenr}[1.3.20 ($*$), \texorpdfstring{\hyperref[loes:12]{Lösung siehe Anhang}}{Lösung siehe Anhang}]\label{aufg:12}\enter
