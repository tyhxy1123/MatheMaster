...
\section{Beispiel: Die Kategorie \texorpdfstring{\ul{Set}}{Set}}\label{section:1.3}
...
\begin{beispiel}[\ul{RelSet}]\label{beisp1.6RelSet}\enter
...
\begin{tikzcd}
                                                                    & b \arrow[rd, "{(b,d)}"] &   \\
a \arrow[rr, "{(a,d)}"] \arrow[ru, "{(a,b)}"] \arrow[rd, "{(a,c)}"] &                         & d \\
                                                                    & c \arrow[ru, "{(c,d)}"] &  
\end{tikzcd}
	$$
	%TODO Add Schlaufen
...
\section{1.9 Diagrammschemata und Diagramme}
\begin{definition}
...
\end{align*} %TODO Add tikzcd. Gut zum Üben @Pascal
