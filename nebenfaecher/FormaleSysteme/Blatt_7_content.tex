...
Die Aussagen $L\cdot L^\ast=L^+$, $L^\ast\cdot L^\ast=L^\ast$ und $L^\ast\cup L=L^\ast$ gelten, denn:
\begin{proof}
	%TODO
\end{proof}
...
Geben Sie für jede der folgenden formalen Sprachen $L_i$ ein Transitionssystem an, das $L_i$
akzeptiert:\nl
Transitionssystem ist NEA mit potenziell mehreren Anfangszuständen und potenziell unendlich vielen Zuständen.

\subsubsection{Aufgabe 5 a)}
$L_0:=\Big\lbrace w\in\lbrace a,\ldots,z\rbrace^\ast~\Big|~w\text{ enthält das Wort ``theorie''}\Big\rbrace$
...
\begin{tikzpicture}[shorten >=1pt,node distance=1.7cm,on grid]
  ...
  \path[->] (q_0) edge [loop above]   node [above] {a,$\ldots$,z} ()
            ...
            (q_7) edge [loop above]   node [above] {a,$\ldots$,z} ();
\end{tikzpicture}

\subsubsection{Aufgabe 5 b)}
$L_1:=\Big\lbrace a^m~\Big|~m\text{ ist ein Vilefaches von }3\Big\rbrace$.\\
...
Für $\Sigma=\lbrace a,b\rbrace$ erzeugt folgender DEA die gewünschte Sprache:\\
\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid]
  \node[state,initial,accepting]   (q_0)                {$q_0$};
  \node[state]           (q_1) [right=of q_0] {$q_1$};
  \node[state] (q_2) [right=of q_1] {$q_2$};
  \node[state] (t)   [below=of q_1] {$q_t$};
  \path[->] (q_0) edge              node [above] {a} (q_1)
  				  edge [bend right=0] node [below] {b} (t)
  			(q_1) edge              node [above] {a} (q_2)
  			      edge [bend right=0] node [right] {b} (t)
  			(q_2) edge [bend right =25] node [above] {a} (q_0)
  			      edge [bend right=0] node [below] {b} (t);
...
Hierfür nutzen wir ein unendliches Transitionssystem:

\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid]
  \node[state,initial,accepting]   (q_0)                {$q_0$};
  \node[state]           (q_1) [right=of q_0] {$q_1$};
  \node[state] (q_2) [right=of q_1] {$q_2$};
  \node[state] (q_3)   [right=of q_2] {$\cdots$};
  \path[->] (q_0) edge              node [above] {a} (q_1)
  			(q_1) edge              node [above] {a} (q_2)
  			      edge [bend right=30] node [above] {b} (q_0)
  			(q_2) edge [bend right =0] node [above] {a} (q_3)
  			      edge [bend right=30] node [above] {b} (q_1)
  			 (q_3) edge [bend right=30] node [above] {b} (q_2)
  			;
\end{tikzpicture}

\subsubsection{Aufgabe 5 d)}
%TODO Tikz Bild (Lukas hat Lösung)
