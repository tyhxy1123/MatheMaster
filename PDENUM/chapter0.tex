...
\end{align*}
$\big\lbrace N_\alpha:|\alpha|\leq r\big\rbrace$ sind unisolvent auf $P_r(K)$.
\begin{align*}
\implies\left.
	\begin{array}{cl}
...
& v\in P_r(K)
\end{array}\right\rbrace\implies v\equiv0
\end{align*}
...
C(K)\cdot\Big(\underbrace{|v+p|_{r+1,p,K}}_{=|v|_{r+1,p,K}}+\sum\limits_{|\alpha|\leq r}\underbrace{|N_\alpha(v+q)|}_{=0, \text{ vorherige Gl.}}\Big)
\end{align*}
\end{proof}
...
&\leq \tilde{C}\cdot |u|_{r+1,p,K}\qquad\forall u\in W^{r+1,p}(K)
\end{align*}
\end{lemma}
\begin{proof}
\begin{align*}
...
\end{lemma}
\begin{proof}
	(Nur Beweisidee, da technisch)
	Nutze folgende Fakten:
\begin{align*}
\hat{I} \cdot \hat{v}&=\sum\limits_{i=1}^m\hat{N}_i(\hat{v})\cdot\hat{\varphi}_i\\
I_K \cdot v&=\sum\limits_{i=1}^m N_i(v)\cdot\varphi \\
...
\begin{align*}
\big| v-I_K\cdot v\big|_{m,q,K}\leq C(\hat{K},\hat{V},\hat{\Sigma})\cdot\big(\meas(K)\big)^{\frac{1}{q}-\frac{1}{p}}\cdot\frac{h_K^{r+1}}{\rho_K^m}|v|_{r+1,p,K}
\end{align*}
für alle $v\in W^{r+1,p}(K)$ wobei $(K,V,\Sigma	)$ affin-äquivalent ist zu $(\hat{K},\hat{V},\hat{\Sigma})$ und
\begin{itemize}
...
\begin{align*}
\big|v-I_K\cdot v\big|_{1,2,K}\leq \hat{C}\cdot h_K^r\cdot |v|_{r+1,2,K}
\end{align*}
\end{theorem}
\begin{proof}
\begin{align*}
\big\Vert \hat{I} \cdot \hat{v} \big\Vert_{m,q,\hat{K}}
...
\end{align*}
Aus $P_r(\hat{K})\subseteq V$ folgt $\hat{I}\cdot\hat{p}=\hat{p}$ für alle $\hat{p}\in P_r(\hat{K})$. Es folgt
\begin{align*}
\hat{v}-\hat{I}\cdot\hat{v}
...
\end{align*}
\begin{align*}
\big| \hat{v}-\hat{I}\cdot \hat{v} \big|_{m,q,\hat{K}}
...
\begin{align*}
&\big(v-I_K\cdot v\big)\circ F_K=\underbrace{v\circ F_K}_{=:\hat{v}}-\hat{I}(v\circ F_K)=\hat{v}-\hat{I}\cdot\hat{v}
\end{align*}
\begin{align*}
\big| v-I_K \cdot v\big|_{m,q,K}
&\stackrel{\ref{theorem4.9}}{\leq}
C\cdot\big\Vert B_K^{-1}\big\Vert^m\cdot\big|\det(B_K)\big|^{\frac{1}{q}}\cdot\big|\underbrace{(v-I_K \cdot v)\circ F_K}_{=\hat{v}-\hat{I} \cdot \hat{v}}\big|_{m,q,K}\\
&\leq C\cdot\big\Vert B_K^{-1}\big\Vert^m\cdot|\det(B_K)|^{\frac{1}{p}}\cdot\big|\hat{v}\big|_{r+1,p,\hat{K}}\\
&\stackrel{\ref{theorem4.9}}{\leq}
C\cdot\big\Vert B_K^{-1}\big\Vert^m\cdot\Vert B_K\Vert^{m}\cdot|\det(B_K)|^{\frac{1}{q}-\frac{1}{p}}\cdot |v|_{r+1,p,K}\\
&\stackrel{\ref{lemma4.10}}{\leq}
C\cdot\frac{\hat{h}^m}{\hat{\rho}^{r+1}}\cdot\frac{h_K^{r+1}}{\rho_K^m}\cdot|\det(B_K)|^{\frac{1}{q}-\frac{1}{p}}\cdot|v|_{r+1,p,K}
\end{align*}
\begin{align*}
