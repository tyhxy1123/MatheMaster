...
\end{align*}
$\Vert\cdot\Vert_h$ ist eine Norm auf $V\cup V_h$.\\
Gleichmäßige Stetigkeit von $a_h$ auf $V\cup V_h$: Seien $v,w\in V\cup V_h$. Dann gilt:
\begin{align*}
\big|a_h(v,w)\big|
...
&\underset{\text{zurück}}{\overset{\text{transf.}}{\leq}} c\cdot h_E^{\frac{1}{2}}\cdot\underbrace{\Vert B_K\Vert^1}_{\sim h_K}\cdot{\underbrace{\big|\det(B_K)\big|}_{\sim h_K^2}}^{-\frac{1}{2}}\cdot|z_h|_{1,2,K}\\
&\leq
...
\begin{align*}
	...
	\text{Aber}&: v\text{ ist nur in }H^1\text{ und die gewöhnliche Interpolation wird nicht funktionieren.}\\
