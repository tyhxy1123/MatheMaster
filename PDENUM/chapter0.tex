...
%RobertToDo: Polishing of the following %TODO
...
\begin{itemize}
	...
	\item %TODO Das hier soll linksbündig stehen und Formelnummer 10 bekommen, mit \label{eq7.3_10}\tag{10}
		% TODO BEARBEITUNG 1: hab einfach den text, der in der VL darunter stand, darüber geschrieben. könnten wir ja so lassen, oder?
		für die eindeutige Lösbarkeit nehmen wir an
...
\begin{proof}
	...
	Stattdessen machen wir partielle Integration:
	\begin{align*}
	...
	\begin{align*}
		...
		&\leq\sum\limits_{K\in\T_h}\delta_K\Big(\varepsilon\underbrace{\Vert\Delta(I_h u-u)\Vert_{0,2,K}}_{\leq c h^{r-1}|u|_{r+1,2,K}}+\Vert b\Vert_{0,\infty,K}\underbrace{|I_h u-u|_{1,2,K}}_{\leq c h^r_K|u|_{r+1,2,K}}+\Vert c\Vert_{0,\infty,K}\underbrace{\Vert I_h u-u\Vert_{0,2,K}}_{\leq c h^{r+1}|u|_{r+1,2,K}}\Big)\\
	...
	\begin{align*}
		...
		\implies \frac{1}{2}\Vert w_h\Vert^2_{\text{SD}}&\leq c h^r\left(\varepsilon^{\frac{1}{2}}+h^{\frac{1}{2}}\right)|u|_{r+1,2,\Omega}\Vert w_h\Vert_{\text{SD}}\\
...
\begin{align*}
	...
	\Bigg(\sum\limits_{K\in\T_h}\underbrace{\approx h_K}\big\Vert b\cdot\nabla(u+h_K)\big\Vert_{0,2,K}\Bigg)^{\frac{1}{2}}
