...
\defi
(Eigenschaften)\enter
Eigenschaften einer Bilinearform $a$:
\begin{itemize}
...
\end{align*}
\end{itemize}
Eigenschaften einer Linearform $l$:
\begin{itemize}
...
\end{align*}
$b\in V^\ast$: $\langle b,v\rangle=b(v)$ Damit folgt:
\begin{align*}
...
\begin{align*}
P:V\to V,\qquad v\mapsto u+\tau\cdot(b-Au)
\end{align*}
...
\textit{Sei $W$ ein Banachraum, $Q:N\to N$ mit $N\subseteq W$ sei eine Kontraktion, d. h.
\begin{align*}
...
&=\langle u-v,u-v\rangle_V-2\cdot\tau\cdot\underbrace{\big\langle A(u-v),u-v\big\rangle_V}_{=a(u-v,u-v)\stackrel{\text{koerz}}{\geq}\alpha\cdot\Vert u-v\Vert^2_V}+\tau^2\cdot\underbrace{\big\langle A(u-v),A(u-v)\big\rangle_V}_{=\Vert A(u-v)\Vert^2_V\leq M^2\cdot\Vert u-v\Vert^2_V}\\
&\stackrel{\tau>0}{\leq}
...
\end{align*}
$P$ ist eine Kontraktion, also $P$ nach dem Banachschen Fixpunktsatz einen eindeutigen Fixpunkt und damit hat $Au=b$ eine eindeutige Lösung.
\end{proof}
...
Sei $V_n\subseteq V$ ein endlich-dimensionaler Unterraum von $V$. Betrachte
\begin{align*}
\text{Finde }u_n\in V_n \text{ so, dass }\qquad
a(u_n,v_n)=l(v_n)\qquad\forall v_n\in V_n
\end{align*}
Da $a$ und $l$ auf $V_n$ die Voraussetzungen des Satzes von Lax-Milgram auf $V_n$ erfüllen, gibt es ein eindeutiges $u_n\in V_n$.\\

Sei $\lbrace\varphi_i\rbrace_{i=1}^n$ eine Basis von $V_n$.
\begin{align*}
\Big(a(u_n,v_n)=l(v_n)\qquad\forall v_n\in V_n\Big)\Longleftrightarrow
...
Sei $V$ ein Hilbertraum, $V_n\subseteq V$ ein abgeschlossener Unterraum, $a$ eine stetige, koerzive Bilinearform auf $V$ und $l$ eine stetige Linearform. Dann gilt:
\begin{align*}
\exists C=\frac{M}{\alpha}>0:\Vert u-u_n\Vert_V\leq C\cdot\inf\limits_{v_n\in V_n}\Vert u-v_n\Vert_V
\end{align*}
\end{theorem}
\begin{proof}
\begin{align*}
a(u,v)&=l(v) &\forall v\in V\\
a(u_n,v_n)&= l(v_n) &\forall v_n\in V_n\subseteq V
\end{align*}
Differenz: 
\begin{align*}
a(u-u_n,v_n)=\qquad v_n\in V_n
\end{align*}
Also:
\begin{align*}
...
&\leq
a(u-u_n,u-u_n)\\
&=a(u-u_n,u-v_v+v_n- u_n)\\
&=a(u-u_n,u-v_n)+\underbrace{a(u-u_n,\underbrace{v_n-u_n}_{\in V_n})}_{=0}\\
&\stackrel{\text{stetig}}{\leq}
M\cdot\Vert u-u_n\Vert_V\cdot\Vert u-v_n\Vert_V\\
\implies
\Vert u-u_n\Vert_V&\leq\frac{M}{\alpha}\cdot\Vert u-v_n\Vert_V\qquad\forall v_n\in V_n\\
\implies
\Vert u-u_n\Vert_V&\leq \frac{M}{\alpha}\cdot\inf\limits_{v_n\in V_n}\Vert u-v_n\Vert_V
\end{align*}
%ToDo: v_n ist eigentlich v_h -.- Das habe ich beim Abschreiben verkackt.
\end{proof}

\begin{theorem}[Dualitätsargument, Aubin-Nietsche]\enter
Sei $V$ ein Hilbertraum, $V_n\subseteq V$ ein abgeschlossener Unterraum, $a$ eine stetige, koerzive Bilinearform und $l$ eine stetige Linearform. Sei weiterhin $H$ ein Hilbertraum mit Skalarprodukt $\langle\cdot,\cdot\rangle_H$ und Norm $\Vert\cdot\Vert_H$ so, dass $V\hookrightarrow H$ und $V$ liegt dicht in $H$ bzgl. $\Vert\cdot\Vert_H$. Sei zusätzlich $u_\varphi\in V$ für jedes feste $\varphi\in H$ die eindeutige Lösung von
\begin{align}\label{Dtheorem2.3}\tag{D}
a(v,u_\varphi)=\langle\varphi,v\rangle_H\qquad\forall v\in V
\end{align}
Dann gilt:
\begin{align*}
\Vert u-u_h\Vert_H\leq M\cdot\Vert u-u_h\Vert_V\cdot\sup\limits_{\begin{subarray}{c}\varphi\in H\\\Vert\varphi\Vert_H\end{subarray}}\inf\limits_{v_h\in V}\Vert u_\varphi-v_h\Vert_V
\end{align*}
\end{theorem}
...
&\leq M\cdot\Vert u-u_h\Vert_V\cdot\Vert u_\varphi-v_n\Vert_V\\
\Vert u-u_h\Vert_H
&=\sup\limits_{\begin{subarray}{c}\varphi\in H\\\Vert\varphi\Vert=1\end{subarray}}\langle u-u_h,\varphi\rangle_H\\
&\leq M\cdot\Vert u-u_h\Vert_V\cdot\sup\limits_{\begin{subarray}{c}\varphi\in H\\\Vert\varphi\Vert_H=1\end{subarray}}\inf\limits_{v_h\in V}\Vert u_\varphi-v_n\Vert_V
\end{align*}
\end{proof}
