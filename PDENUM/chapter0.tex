...
\begin{align}\nonumber
	...
	\qquad\forall K\in\T_h,v_h\in V_h
	\label{eq7.2_2}\tag{2}\\
	\mit (\varphi,\psi):&=\int\limits_K\varphi(x)\cdot\psi(x)\d x
	\nonumber
\end{align}
...
\begin{align}\label{eq7.2_4}\tag{4}
	a_h(u,v_h)&=f_h(x_h)\qquad\forall v_h\in V_h
\end{align}
wobei 
...
%TODO Bin mir nicht sicher ob $p_{e_K}$ gemeint ist. Bitte fixen.
\begin{align*}
	\text{pe}_K:=\frac{\Vert b\Vert_{0,\infty,K}}{2\cdot\varepsilon}\cdot h_K
\end{align*}
...
\begin{align}\label{eq7.2_9}\tag{9}
	\delta_K:=\left\lbrace\begin{array}{cl}
		\delta_0 h_K, &\falls \text{pe}_K>1\\
		\frac{\delta_1 h_K^2}{\varepsilon}, &\falls\text{pe}_K\leq 1	
	\end{array}\right.
\end{align}
...
\begin{itemize}
	...
	\item %TODO Das hier soll linksbündig stehen und Formelnummer 10 bekommen, mit \label{eq7.3_10}\tag{10}
...
\begin{lemma}[Koerzivität]\label{lemma7.1Koerzivitaet}
	Sei
	\begin{align*}
		c_k:=\Vert v\Vert_{0,\infty,\Omega}=\max\limits_{x\in K}\big|c(x)\big|>0
	\end{align*}
	und
	\begin{align}\label{eq7.3_13}\tag{13}
		0<\delta_K\leq\frac{1}{2}\cdot\min\left\lbrace\frac{c_0}{c_K^2},\frac{h_K^2}{\varepsilon\cdot\mu_{\text{inv}}}\right\rbrace
	\end{align}
...
\begin{proof}
	...
	\begin{align*}
		...
		+\underbrace{\sum\limits_{K\in\T_h}\delta_K\big(c v_h,b\cdot\nabla v_h\big)_K}_{=:T_2}\\
		...
		\overset{\text{CS}}{\leq}
		\sum\limits_{K\in\T_h}\delta_K\varepsilon\Vert\Delta v_h\Vert_{0,2,K}\cdot\Vertb\cdot\nabla v_h\Vert_{0,2,K}\\
	\end{align*}
	Mit
	\begin{align*}
		\alpha\beta
	...
	\begin{align*}
		|T_2|
		&\leq\sum\limits_{K\in\T_h}\delta_K\Big|(c v_h,b\cdot\nabla v_h)_K\Big|\\
...
\begin{align*}
	...
	\implies \Vert u_h\Vert_{\text{SD}}
	\leq C(f)
\end{align*}

Wir habe zusätzliche Kontrolle über die Ableitungen in Streamline-Richtung
\begin{align*}
	\left(\sum\limits_{K\in\T_h}\underbrace{\delta_K}_{\approx h_K\falls \text{pe}_K>1}\Vert b\cdot\nabla u_h\Vert^2_{0,2,K}
\end{align*}
%TODO Ende fehlt