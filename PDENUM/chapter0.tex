...
\begin{figure}[!ht]
	...
	\end{center}
\end{figure}
%TODO Hier Skizze einfügen (muss aber nicht sein. Ist wohl nicht so wichtig)
...
\begin{itemize}
\item $a_h$ sei Bilinearform auf $V\cup V_h$ %kein Plan warum er hier $V+V_h$ schreibt. Ich ziehe das mal konsequent durch.
\item $a_h$ sei \textbf{gleichmäßig stetig auf $V\cup V_h$}, d. h.
\begin{align*}
\exists M>0\text{ unabhängig von }h:\big|a_h(u,v)\big|\leq M\cdot\Vert u\Vert_h\cdot\Vert v\Vert_h\qquad\forall u,v\in V\cup V_h
\end{align*}
...
Gleichmäßige Stetigkeit von $a_h$ auf $V+V_h$: Seien $v,w\in V+V_h$. Dann gilt:
\begin{align*}
\big|a_h(v,w)\big|
...
&=\Vert v\Vert_h\cdot\Vert w\Vert_h
\implies M=1
\end{align*}
...
%TODO Beim folgenden bin ich unsicher ob \leq oder \geq
\begin{align*}
\big|f_h(v_h)\big|
\leq
...
\Vert u-u_h\Vert_h
&\leq
2\cdot\underbrace{\inf\limits_{v_h\in V_h}\Vert u-v_h\Vert_h}_{\leq c\cdot h\cdot |u|_{2,2}}+1\cdot\underbrace{\sup\limits_{z_h\cdot V_h}\frac{a_h(u,z_h)-f_h(z_h)}{\Vert z_h\Vert_h}}_{\leq?}
\end{align*}
...
Sei $\overline{z}_h$ und $\overline{\frac{\partial u}{\partial u_E}}$ die konstanten Integral-Mittelwerte von $z_h$ und $\frac{\partial u}{\partial u_E}$ auf jedem $E$, formal:
\begin{align*}
\overline{z}_h|_E
:&=\frac{1}{|E|}\cdot\int\limits_E z_h\d\gamma,\qquad\left.\overline{\frac{\partial u}{\partial u_E}}\right|_E:=\frac{1}{|E|}\cdot\frac{\partial u}{\partial u_E}\d\gamma
\end{align*}
Mit 
\begin{align*}
...
folgt
\begin{align*}
\int\limits_E\frac{\partial u}{\partial u_E}[z_h]\d\gamma
...
\implies
L_h(z_h)
&=\sum\limits_{K\in\T_h}\sum\limits_{E\subseteq\partial K}\int\limits_E\left(\frac{\partial u}{\partial u_K}-\overline{\frac{\partial u}{\partial u_K}}\right)\big(z_h-\overline{z}_h\big)\d\gamma\\
\overset{\text{CS}}&\leq
\sum\limits_{K\in\T_h}\sum\limits_{E\subseteq\partial K}\left\Vert\frac{\partial u}{\partial u_K}-\overline{\frac{\partial u}{\partial u_K}}\right\Vert_{0,2,E}\cdot\Vert z_h-\overline{z}_h\Vert_{0,2,E}
\end{align*}
Berechne nun die Differenz: $z_h-\overline{z}_h$:
\begin{align*}
\big\Vert z_h-\overline{z}_h\big\Vert_{0,2,E}
&\leq
c\cdot\underbrace{\big\Vert B_E^{-1}\big\Vert^0}_{=1}\cdot\underbrace{\big|\det(B_E)\big|}_{=h_E}^{\frac{1}{2}}\cdot\big\Vert\underbrace{ \hat{z}_h-\hat{\overline{h}}_h}_{=\widehat{z_h-\overline{z}_h}\big\Vert_{0,2,\hat{E}}}\\
&\leq
c\cdot h_E^{\frac{1}{2}}\cdot\big\Vert\widehat{z_h-\overline{z}_h}\big\Vert_{0,2,\hat{E}}
\end{align*}
Der Operator
\begin{align*}
L:\hat{H}(\hat{K})\to L_2(\hat{E}),\qquad\hat{v}\mapsto\left(\hat{v}-\overline{\hat{v}}\right)|_{\hat{E}}
...
&\leq
\frac{1}{|\hat{E}|^2}\cdot\Big(\underbrace{\Vert1\Vert_{0,2,\hat{E}}}_{=|\hat{E}|^{\frac{1}{2}}}\cdot\Vert\hat{v}\Vert_{0,2,\hat{E}}\Big)^2\cdot|\hat{E}|=\Vert\hat{v}\Vert^2_{0,2,\hat{E}}\leq c\cdot\Vert\hat{v}\Vert_{1,2,\hat{K}}\\
\implies
\big\Vert L(\hat{v})\big\Vert_{0,2,\hat{E}}
...
&\leq c\cdot h_E^{\frac{1}{2}}\cdot\underbrace{\Vert B_K\Vert^1}_{\sim h_K}\cdot{\underbrace{\big|\det(B_K)\big|}_{\sim h_K^2}}^{-\frac{1}{2}}\cdot|z_h|_{1,2,K}\\
&\leq
c\cdot h_E^{\frac{1}{2}}\cdot|z_h|_{1,2,K}
\end{align*}
Auf ähnliche Art und Weise, aber mit wesentlich mehr Aufwand, erhält man auch:
\begin{align*}
\left\Vert\frac{\partial u}{\partial u_K}-\overline{\frac{\partial u}{\partial u_h}}\right\Vert_{0,2,E}
...
\begin{align*}
\big\Vert u-u_h\big\Vert_h\leq c\cdot h^2\cdot\Vert u\Vert_{2,2,\Omega}
\end{align*}
\end{bemerkung}

\section{Eine posteriori Fehlerabschätzung} %6
...
\begin{align}\label{eqSecttion6Ph}\tag{$P_h$}
a(u_h,v_h)=l(v_h)\qquad\forall v_h\in X_n
\end{align}
...
&=\sum\limits_{T\in\T_h}\left(\int\limits_T f\cdot v\d x+\int\limits_T\Delta u_h\cdot v\d x-\int\limits_{\partial T}\nabla u_h\cdot n_T\cdot v\d\gamma\right)\\
&=\sum\limits_{T\in\T_h}\int\limits_T\big(f+\Delta u_h\big)\cdot v\d x
+\sum\limits_{E\in\mathcal{E}_h}\int\limits_E\big[\nabla u_h\cdot n_E\big]_E\cdot v\d\gamma\\
\overset{\text{CS}}&\leq
\sum\limits_{T\in\T_h}\big\Vert f+\Delta u_h\big\Vert_{0,2,T}\cdot\Vert t\Vert_{0,2,T}+\sum\limits_{E\in\mathcal{E}_h}\Big\Vert\big[\nabla u_h\cdot n_E\big]_E\Big\Vert_{0,2,E}\cdot\Vert v\Vert_{0,2,E}\\
R(v)
&=R(v-v_h)\\
&\leq
...
Bester Weg: Wähle $v_h$ als Interpolation von $v$.\\
Aber: $v$ ist nur in $H^1$ und die gewöhnliche Interpolation wird nicht funktionieren.\\
Lösung: Quasi-Interpolation.\\
Hier: Quasi-Interpolation von Clément
...
\item $\overline{w}_z:=\inner\left(\bigcup\limits_{\begin{subarray}{c}T\in\T_h\\z\text{ ist Knoten von }V\end{subarray}}\overline{T}\right)$
