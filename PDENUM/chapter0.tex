...
\begin{align*}
V_h:=\Bigg \lbrace v&=(v_{K})_{K\in\T_h}\in\prod\limits_{K\in\mathcal{T}_h} V(K):N_i^K(v|_K)=N_j^{K'}(v|_{K'})~\\
&{} \quad\forall N_i^K,N_j^{K'}\in\Lambda(N),\forall N\in\Sigma_h\Bigg \rbrace
\end{align*}
...
\begin{bemerkung}
Die Dualitätsargument von Aubin und Nietsche kann erweitert werden auf den nichtkonformen Fall. Dabei tauchen zwei neue Terme in der Abschätzung auf.
\begin{align*}
