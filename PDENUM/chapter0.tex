...
|l(v)|&\leq\Vert f\Vert_{0,2}\cdot\Vert v\Vert_{0,2}\leq\Vert f\Vert_{0,2}\cdot\Vert v\Vert_{1,2}
\end{align*}
Bilinearform
\begin{align*}
...
&\leq
\Vert A\Vert_{L^\infty}\cdot\Vert\nabla v\Vert_{0,2}\cdot\Vert\nabla w\Vert_{0,2}+\Vert a\Vert_{L^\infty} \cdot\Vert\nabla v\Vert_{0}\cdot\Vert w\Vert_{0,2}+\Vert\alpha\Vert_{L^\infty}\cdot\Vert v\Vert_{0,2}\cdot\Vert w\Vert_{0,2}\\
&\leq
...
Koerzivität: 
\begin{align*}
a(v,w)&\geq\underbrace{\beta}_{>0}\cdot\Vert v\Vert^2_{1,2}\qquad\forall v\in V=H_0^1(\Omega)\\
a(u,v)&=
\int\limits_\Omega\nabla v^T\cdot A\cdot\nabla v+\underbrace{(a\cdot\nabla v+\alpha\cdot v)\cdot v}_{=\underbrace{(a\cdot\nabla v)\cdot v}_{=\frac{\alpha}{2}\cdot\nabla(v^2)}+\alpha\cdot v^2}\d x\\
&=\int\limits_\Omega\underbrace{\nabla v^T\cdot A\cdot\nabla v}_{\geq\lambda_0\cdot|\nabla v(x)|^2\geq0}+\underbrace{\big(\alpha-\frac{1}{2}\cdot\div(a)\big)}_{\stackrel{\text{Vor}}{\geq}0}\cdot v^2\d x+\underbrace{\int\limits_{\Gamma}\frac{1}{2}\cdot(a\cdot u)\cdot v^2\d\gamma}_{=0}\\
&\geq
...
\begin{align*}
\int\limits_\Gamma\frac{1}{2}\cdot(a\cdot u)\cdot v^2\d\gamma
&=\underbrace{\int\limits_{\Gamma_D}\frac{1}{2}\cdot(a\cdot u)\cdot v^2\d\gamma}_{=0}+\underbrace{\int\limits_{\Gamma_N}\frac{1}{2}\cdot\underbrace{(a\cdot u)}_{\geq0}\cdot v^2\d\gamma}_{\geq0}
\end{align*}
...
\begin{align*}
a(v,v)&=\int\limits_\Omega\nabla v^T+\underbrace{\alpha}_{\geq\alpha_0>0}\cdot v^2\d x\\
&\geq\lambda_0\cdot|v|^2_{1,2,\Omega}+\alpha_0\cdot\Vert v\Vert^2_{0,2,\Omega}\geq\underbrace{\min\lbrace\lambda_0,\alpha_0\rbrace}_{>0}\cdot\Vert v\Vert^2_{1,2,\Omega}
\end{align*}
Stetigkeit von $a_1$:
\begin{align*}
...
&=\int\limits_{\Omega}\nabla v^T\cdot A\cdot\nabla v+\underbrace{\left(\alpha-\frac{1}{2}\cdot\div(a\right)}_{\geq0}\cdot v^2\d x+\int\limits_\Gamma\frac{1}{2}\cdot\underbrace{(a\cdot u)}_{\geq0}\cdot v^2\d\gamma\\
...
&\geq\underbrace{\alpha_0}{>0}\cdot\Vert c^2\Vert^2_{0,2,\Omega}\\
...
\underline{Zeige (iv):}
\begin{align*}
a\cdot n&=0\text{ auf }\Gamma,\qquad\alpha=0,~\div(a)=0\text{ in  }\Omega\\
...
0=-\int\limits_\Omega\underbrace{(\div(a))}_{\equiv0}\cdot u\d x+\int\limits_\Gamma\underbrace{(a\cdot u)}_{\equiv 0}\cdot u\d\gamma
\end{align*}
\end{proof}

\begin{theorem}[Regularität]\label{theorem3.3}\enter
Sei $\Gamma$ eine $C^1$-Mannigfaltigkeit oder $\Omega$ konvex. Wenn die Neumann-Randbedingungen erfüllt sind, nehmen wir an, dass es eine Funktion 
\begin{align*}
