...
\begin{align*}
S_n^{k,-1}&:=\Big\lbrace\varphi:[0,1]\to\R:\varphi\big|_{I_j}\in P_k,~0\leq j\leq n\Big\rbrace\\
...
\end{aligned}$ wegen Proposition 1.4 %ToDo: add reference
\item $\begin{aligned}
S_h^{0,m}
...
\begin{align*}
S^{1,0}_{h,0}&=\big\lbrace\varphi_i:1\leq i\leq n\big\rbrace,\qquad
\varphi_i(t)=\left\lbrace\begin{array}{cl}
\frac{t-t_{i-1}}{h_{i-1}}, &\falls t\in I_{i-1}\\
\frac{t_{i+1}-t}{h_i}, &\falls t\in I_i\\
0, &\sonst
\end{array}\right.\\
&\implies a_{i,j}=0\text{ für }|i-j|>1\implies A\text{ ist Dreiecksmatrix}
\end{align*}
...
&\leq h_i\cdot\left|u-v_h^\ast\right|_{1,2,I_i}\\
\implies
