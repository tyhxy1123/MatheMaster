...
\begin{itemize}
\item Gegeben ist eine Membran als Graph der Funktion $u:\Omega\to\R$ wobei $\Omega\subseteq\R^2$
...
\begin{align*}
	...
	\gdw ~ E(u)&\leq E(u+t\cdot v) \quad\forall t\in\R,~&\forall v\in V
\end{align*}
...
&=\frac{\d}{\d t}\int\limits_\Omega\left(\frac{1}{2}\cdot\left((u+t\cdot v)_x^2+(u+t\cdot v)_y^2-f(u+t\cdot v)\right)\d x\d y\\
...
\begin{satz}[Glätte von stückweise glatten Funktionen]
Seien $\Omega_1,\Omega_2\subseteq\Omega$ zwei nichtleere, offene, beschränkte und disjunkte Teilmengen mit stückweise glattem Rand. Gelte $\overline{\Omega}=\overline{\Omega}_1\cup\overline{\Omega}_2$.
...
$\Omega$ hat einen \textbf{Lipschitz-Rand} $:\gdw\\\exists N\in\N$ und es offene Mengen $U_1,\ldots,U_N\subseteq\R^d:$ gibt, sodass
\begin{enumerate}
\item $
\begin{aligned}
...
\begin{enumerate}
\item Eine lineare Abbildung $A:X\to Y$ heißt \textbf{kompakt} $:\gdw$ das Bild der abgeschlossenen Einheitskugel in $X$ ist im Raum $Y$ abgeschlossen.
