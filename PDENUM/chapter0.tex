...
%TODO fix k: hier großes und kleines K? Wo kommt welches hin?
\begin{definition}[gobales Nodal-Funktional / globale Freiheitsgrade (dof)]\enter
Seien $N_i^k$ und $N_j^{k'}$ zwei locale Nodal-Funktionale von den finiten Elementen $(K,V,\Sigma)$ und $(K',V',\Sigma')$. Wir sagen, diese beiden lokalen Freiheitsgrade \textbf{gehören zum selben globalen Freiheitsgrad}
\begin{align*}
...
\begin{align*}
F:\hat{K}\to K,\qquad \hat{x}\mapsto B_K\hat{x}+ b_K\qquad\mit B_k\in\R^{d\times d}\text{ invertierbar und }b_k\in\R^d
\end{align*}
Falls $v\in W^{m,p}(K)$ für $m\geq0,p\in[1,\infty)$, dann gehörte $\hat{v}:=v\circ F$ zu $W^{m,p}(\hat{K})$ und wir erhalten
\begin{align*}
...
\begin{proof}
\begin{align*}
\Vert B_K\Vert&=\sup\limits_{z\neq0}\frac{\Vert B_k z\Vert}{\Vert z\Vert}=\frac{1}{\hat{\rho}}\cdot\sup\limits_{\Vert z\Vert=\hat{\rho}}\Vert B_k z\Vert
\end{align*}
Für alle $y$ mit $\Vert y\Vert=\hat{\rho}$ existieren zwei Punkte $\hat{x},\hat{y}\in\hat{K}$ so, dass $y=\hat{x}-\hat{y}$. Seien $x=F(\hat{x}),y=F(\hat{y})$. Dann gilt:
\begin{align*}
x-y&=B_k\hat{x}+b_K-\big(B_k\hat{y}+b_K\big)=B_k(\hat{x}-\hat{y})=B_K y\\
\Vert B_K y\Vert&=\Vert x-y\Vert\leq h_K\\
\implies \Vert h_K\Vert&\leq\frac{1}{\hat{\rho}}\cdot\sup\limits_{\Vert y\Vert=\hat{\rho}}\underbrace{\Vert B_k y\Vert}_{\leq h_K}\leq\frac{h_K}{\hat{\rho}}
\end{align*}
\end{proof}