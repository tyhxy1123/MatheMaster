...
% Das war ein guter Moment :D Lassen wir das als easter egg im Code?
\begin{proof}
...
\end{align*}
%TODO Oben habe ich sicher einige Male \omega und w vertauscht. Bitte fixen. Aßerdem weiß ich nicht, ob man z groß oder klein schreiben sollte.
...
\begin{align*}
R(v)&\leq
c\cdot\sum\limits_{T\in\T_h}\big\Vert f+ g u_h\big\Vert_{0,2,T}\cdot h_T\cdot |v|_{1,2,\tilde{\omega}_T}\\
...
&\quad+c\cdot\left(\sum\limits_{E\in\Sigma_h}\big\Vert[\nabla u_h\cdot n_E]_E\big\Vert^2_{0,2,E}\right)\cdot\underbrace{\left(\sum\limits_{E\in\Sigma_h}|v|^2_{1,2,\tilde{\omega}_E}\right)^{\frac{1}{2}}}_{\leq c\cdot |v|_{1,2,\Omega}\text{, $c$ aus Formregulärität}}\\
&\leq
c\cdot\left(\sum\limits_{T\in\T_h} h^2_T\cdot\big\Vert f+\Delta u_h\big\Vert^2_{0,2,T}
+\sum\limits_{E\in\Sigma_h}h_E\cdot\big\Vert\nabla u_h\cdot v_E]_E\big\Vert^2_{0,2,E}\right)^{\frac{1}{2}}\cdot\Vert v\Vert_{1,2,\Omega}\nl
&\implies
\Vert u-u_h\Vert_{1,2,\Omega}
\leq
c\cdot\sup\limits_{\begin{subarray}{c}v\in H_0^1(\Omega)\\\Vert v\Vert_{1,2,\Omega}=1\end{subarray}}R(v)\\
&\hspace{58pt}\leq
 c\cdot\underbrace{\left(\sum\limits_{T\in\T_h} h^2_T\cdot\big\Vert f+\Delta u_h\big\Vert^2_{0,2,T}
% Undo Henriks changes because of overfull box warning
+\sum\limits_{E\in\Sigma_h}h_E\cdot\big\Vert\nabla u_h\cdot v_E]_E\big\Vert^2_{0,2,E}\right)^{\frac{1}{2}}}_{=\eta}
\end{align*}
Setze
\begin{align*}
...
\hspace{48pt}
\max\limits_{x\in\overline{\omega_E}}\psi_E(x)=1
\end{aligned}$
\item $\begin{aligned}
\hspace{120pt}
\psi_E\in C(\omega_E)
...
\end{enumerate}
\end{lemma}
%TODO Manchmal schreibe ich \Sigma_h statt \mathcal{E}_h ode rumgekehrt. Bitte fixen
\begin{theorem}
...
\begin{align*}
%Undo Henriks changes because of overfull box
...
&=\int\limits_T\big(f+\Delta u_h\big)\cdot v_T\d x+\int\limits_T\big(f_h-f\big)\cdot v_T\d x+\int\limits_{\partial T}\nabla u_h\cdot n_T\cdot\underbrace{v_T}_{=0}\d\gamma\\
\overset{\text{part Int}}&=
\int\limits_T\big(\nabla u-\nabla u_h\big)\cdot\nabla v_T\d x+\int\limits_T\big(f_h-f\big)\cdot v_T\d x\\
\overset{\text{CS}}&\leq
\big|u-u_h\big|_{1,2,T}\cdot\big|v_T\big|_{1,2,T}+\big\Vert f-f_h\big\Vert_{0,2,T}\cdot\big\Vert v_T\big\Vert_{0,2,T}\\
&\leq
c\cdot h_T^{-1}\cdot\big\Vert u-u_h\big\Vert_{1,2,T}\cdot\big\Vert\underbrace{v_T}_{=\psi_T\big(f_h+\Delta u_h\big)}\big\Vert_{0,2,T}+\big\Vert f-f_h\big\Vert_{0,2,T}\cdot\big\Vert v_T\big\Vert_{0,2,T}\\
&\leq
c\cdot h^{-1}_T\cdot\big\Vert u-u_h\big\Vert_{1,2,T}\cdot\big\Vert f_h+\Delta u_h\big\Vert_{0,2,T}+\big\Vert f-f_h\big\Vert_{0,2,T}\cdot\big\Vert f_h+\Delta u_h\big\Vert_{0,2,T}
\end{align*}
...
&\leq
\big\Vert u-u_h\big\Vert_{1,2,\omega_E}\cdot\big|v_E\big|_{1,2,\omega_E}+\left(\sum\limits_{T\in\omega_E}\big\Vert f+\Delta u_h\big\Vert^2_{0,2,T}\right)^{\frac{1}{2}}\cdot\big\Vert v_E\big\Vert_{0,2,\omega_E}\\
&\leq
\big\Vert u-u_h\big\Vert_{1,2,\omega_E}\cdot c_6\cdot c_7\cdot h_E^{-\frac{1}{2}}\cdot\big\Vert[\nabla u_h\cdot n_E]\big\Vert_{0,2,E}\\
&{}\quad +h_E^{\frac{1}{2}}\cdot\left(\sum\limits_{T\in\omega_E}\big\Vert f+\Delta u_h\big\Vert^2_{0,2,T}\right)^{\frac{1}{2}}\cdot\big\Vert[\nabla u_h\cdot n_E]_E\big\Vert_{0,2,E}
\end{align*}
\begin{align*}
\Rightarrow h_E^{\frac{1}{2}}\cdot\big\Vert[\nabla u_h\cdot n_E]_E\big\Vert_{0,2,E}
...
\item Wähle eine Start-Triangulierung $T_0$, Setze $l:=0$
\item Löse das diskrete Problem auf $T_l$
\item Berechne $\eta_T$, $T\in\T_l$
\begin{align*}
\eta_l:=\max\limits_{T\in\T_l}\eta_T,\qquad\eta:=\left(\sum\limits_T\eta_T^2\right)^{\frac{1}{2}}
\end{align*}
\item Falls $\eta_l\leq\varepsilon$ STOP
\item Verfeinere alle Zellen $T$ mit $\eta_T\geq\gamma\eta_l$\\
...
\begin{itemize}
\item rote Verfeinerung: normale Verfeinerung: Unterteile ein Dreick in 4 Teildreicke
%TODO Skizze hier ergänzen 
\item grüne Verfeinerung: verbinde den Mittelpunkt der \ul{längsten} Kante mit der entgegengesetzten Ecke
%TODO Skizze hier ergänzen
\item blaue Verfeinerung: Verbinde den Mittelpunkt der \underline{längsten} Kante mit der entgegengesetzten Ecke und mit einem Mittelpunkt einer anderen Kante
%TODO Skizze hier ergänzen
\end{itemize}
...
%TODO Skizze hier ergänzen (Das Rechteck, was zerlegt wird)
%TODO noch mehr Skizzen, Skizzen hier, Skizzen da, SKIZZEN EVERYWHERE!!!!1!elf!einself!

\section{Streamline-Diffusions-Methode (SDFEM)} %7.
Hughes / Brooks 1979

\subsection{Motivation}
\begin{align*}
-\varepsilon\cdot\Delta u+b\cdot\Delta u+c\cdot u&=f\text{ in }\Omega\\
...
\end{align*}
%TODO Skizze hier ergänzen (mit kleinem und großem Epsilon)

ersetze $\varepsilon$ durch Gitterparameter $h$.\\
$\implies$ keine Oszillation, aber "Schmiererei" %smearing

%TODO Skizze hier ergänzen 

Schmiererei hauptsächlich in Seitwindrichtung %TODO fix translation 
