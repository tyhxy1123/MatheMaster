...
\begin{lemma}[Fortin, 1977]\label{lemmaFortin}\enter
	...
	\begin{align*}
		\inf\limits_{q\in Q}\sup\limits_{v\in V}\frac{b(v,q)}{\Vert v \Vert\Vert q\Vert_Q}\geq\beta>0
	\end{align*}
	gilt. Dann gilt die diskrete inf-sup-Bedingung
	\begin{align*}
		\inf\limits_{q_h\in Q_h}\sup\limits_{v_h\in V_h}\frac{b(q_h,v_h)}{\Vert v_h \Vert\Vert q_h\Vert_Q}\geq\beta^\ast>0
	\end{align*}
	...
	\begin{align}\label{eqLemmaFortinBed1}
		b(q_h,v)=b(q_h,I_h v)\qquad\forall q_h\in Q,\forall v\in V
	\end{align}
	und 
...
\begin{proof}\
	\begin{enumerate}[label=(\roman*)]
		\item Angenommen es gibt $I_h$ mit den beiden obigen Eigenschafen \eqref{eqLemmaFortinBed1}, \eqref{eqLemmaFortinBed2}.
		Sei $q_h\in Q$ beliebig.
		\begin{align*}
			\sup\limits_{v_h\in V_h}\frac{b(q_h,v_h)}{\Vert v_h \Vert\Vert q_h\Vert_Q}
		...
		Wir haben als die diskrete inf-sup-Bedingung (u.a.) aus der stetigen inf-sup-Bedingung gefolgert.
		...
		\end{align*}
		Also ist $B_h:W_h^\perp\to Q_h^\ast$ ein Isomorphismus.
		\begin{align*}
			\Vert v_h\Vert_V
			&\leq\frac{1}{\beta^\ast}\Vert B_h v_h\Vert_{Q_h^\ast}
			\overset{\text{Def Norm}}=
			\frac{1}{\beta^\ast}\sup\limits_{q_h\in Q_h}\frac{b(q_h,v_h)}{\Vert q_h\Vert_Q}
		\end{align*}
		...
		wobei
		\begin{align*}
			\langle q_h,q_h\rangle:=b(q_h,v)\qquad\forall q_h\in Q_h
		\end{align*}
		Also gilt
		\begin{align*}
			b(q_h,I_h v)
...
\begin{bemerkung}
	In nichtkonforme Räume $V_h$ ändert sich folgendes:
	\begin{align*}
...
Sei $V_h$ $P_1^{hc}$ das Crouzeix-Raviart-Element und $Q_h$ stückweise konstantes $P_0$.
Wir wollen Lemma \ref{lemmaFortin} anwenden. 
Wie wählen wir $I_h\colon V_h\to V$?\nl
Der natürliche Weg ist folgender:
\begin{align*}
	\frac{1}{|E|}\int\limits_E I_h v\d\gamma=\frac{1}{|E|}\int\limits c\d\gamma=(I_h v)(m_E)
\end{align*}
...
\begin{align*}
	...
	\overset{\text{Gauß}}&=
	\sum\limits_{K} q_K\int\limits_{\partial K} \div(I_h v)\cdot n_K\d \gamma\\
	&=\sum\limits_K q_K\sum\limits_{E\subseteq\partial K}\int\limits_E(I_h v)\cdot n_K\d\gamma\\
	\overset{\text{Def }I_h}&=
	\sum\limits_K q_K\sum\limits_{E\subseteq\partial K}\int\limits_E v\cdot n_K\d\gamma\\
