...
\begin{proof}
Sei $\mathcal{N}(T)$ die Menge alles Ecken von $T$. Dann gilt:
\begin{align*}
\sum\limits_{Z\in\mathcal{N}(T)}\varphi_z|_T=1
\end{align*}
\begin{enumerate}[label=\roman*)]
\item $\begin{aligned}
\int\limits_{w_z}\big(v-\pi_z(v)\big)\d x=0
\end{aligned}$
\begin{align*}
\big\Vert v-\pi_z(v)\big\Vert_{0,2,w_z}
\overset{\ref{korollarPoincareUngleichung}}&\leq
c_P\cdot\underbrace{\diam(w_z)}_{\leq c_3\cdot h_T}\cdot\underbrace{\big|v-\pi_z(v)\big|_{1,2,w_z}}_{=|v|_{1,2,w_z}}
=c_4\cdot h_T\cdot|v|_{1,2,w_z}
\end{align*}
\item $\begin{aligned}
...
\end{aligned}$\\
$T$ hat keine Ecken auf $\partial\Omega=\Gamma$. Also gilt:
\begin{align*}
...
&\leq
\sum\limits_{z\in\mathcal{N}(T)}\Big\Vert\big(v-\pi_z(v)\big)\Big\Vert_{0,2,w_z}\\
&\leq
c_4\cdot h_T\cdot\sum\limits_{z\in\mathcal{N}(T)}|v|_{1,2,w_z}\\
&\leq
3\cdot h_T\cdot|v|_{1,2,\tilde{\omega}_T}\\
\end{align*}
...
_{
\begin{array}{l}
\leq\Vert v-\pi_z(v)\Vert^2_{0,2,w_z}\\
\leq c_4^2\cdot h_T^2\cdot|v|^2_{1,2,w_z}
\end{array}}+h_T^{+1}\cdot\underbrace{\big|v-\pi_z(v)\big|^2_{1,2,T'}}_{=|v|^2_{1,2,T'}}\Big)\\
&\leq
c_8^2\cdot|v|^2_{1,2,w_z}
\leq
c_8^2\cdot|v|_{1,2,\tilde{w}_T}^2\\
&\implies
\Big\Vert\big(\pi_z(v)\big)\Big\Vert\leq c_9\cdot h_T\cdot|v|_{1,2,\tilde{w}_T}
\end{align*}
\item  Sei $E$ eine Kante, die keinen Knoten auf $\Gamma$ hat. Dann gilt:
\begin{align*}
\big\Vert v-R_h(v)\big\Vert_{0,2,E}
&\leq
c\cdot\Big(h_T^{-\frac{1}{2}}\cdot\underbrace{\big\Vert h-R_h(v)\big\Vert_{0,2,T}}_{=c\cdot h_T\cdot|v|_{1,2,\tilde{w}_T}}+h_T^{\frac{1}{2}}\cdot\underbrace{\big|v-R_h(v)\big|_{1,2,T}}_{=|v|_{1,2,T}}\Big)\\
\big\Vert v-R_h(v)\big\Vert_{0,2,E}
...
&\leq
c_{10}\cdot\sum\limits_{z\in\mathcal{N}(E)}\Big(h_T^{-\frac{1}{2}}\cdot\underbrace{\big\Vert v-\pi_z(v)\big\Vert_{0,2,T}}_{\leq c\cdot h_T\cdot|v|_{1,2,w_z}}+h_T^{\frac{1}{2}}\cdot\underbrace{\big|v-\pi_z(v)\big|_{1,2,T}}_{\leq|v|_{1,2,w_z}}\Big)\\
&\leq
c\cdot h_E^{\frac{1}{2}}\cdot|v|_{1,2,\tilde{w}_E}
\end{align*}
\item $E$ hat Knotenbeschränkung %TODO bitte Übersetzung fixen
\end{enumerate}
\end{proof}
