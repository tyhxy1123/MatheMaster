...
\end{lemma}
\begin{proof}
\begin{align*}%%% das sieht scheiße aus
...
&\leq c\cdot\big\Vert B_K^{-1}\big\Vert\cdot|\det(B_K)|^{\frac{1}{p}}\cdot\big|\hat{v}\big|_{r+1,q,\hat{K}}\\ %%%  \hat ?
&\stackrel{\ref{theorem4.9}}{\leq}
...
c\cdot\frac{\hat{h}^m}{\hat{\rho}^{r+1}}\cdot\frac{h_K^{r+1}}{\rho^m_K}\cdot|\det(B_K)|^{\frac{1}{q}-\frac{1}{p}}\cdot|v|_{r+1,q,K}\\
...
Eine Familie $\lbrace\T_h\rbrace_{h\in I}$ von geeigneten Triangulierungen vom $\Omega$  heißt \textbf{formregulär}
$:\gdw$
\begin{enumerate}[label=(\roman*)]
\item $\begin{aligned}
\exists\sigma>0:\forall\T_h:\forall K\in\T_h:\frac{h_K}{\rho_K}\leq\sigma
\end{aligned}$
\item Der Familienparameter geht nach 0. %TODO Formalisieren! Was bedeutet das?
\end{enumerate}
\end{definition}
...
\begin{align*}
\big| v-I_K v\big|\leq\tilde{c}\cdot\big(\meas(K)\big)^{\frac{1}{q}-\frac{1}{p}}\cdot h_K^{r+1-n}\cdot|v|_{r+1,p,K}
\end{align*}
...
\begin{align*}
\big\Vert v-I_K v\big\Vert\leq\tilde{c}\cdot\big(\meas(K)\big)^{\frac{1}{q}-\frac{1}{p}}\cdot h_K^{r+1-n}\cdot|v|_{r+1,p,K}
\end{align*}
\end{theorem}
...
\end{theorem}
\begin{proof}
Céas Lemma \ref{theorem2.2CeasLemma} liefert
\begin{align*}
...
&=c\cdot h^{2\cdot r}\cdot|u|^2_{1,2,\Omega}
\end{align*}
\end{proof}
...
\end{align*}
erfüllt. Dann existiert eine Konstante $c$, unabhängig vo n$h$ so, dass
\begin{align*}
...
Diese Abschätzungen werden \textbf{a priori Abschätzungen} genannt.

\begin{lemma}[Inverse Abschätzung]\label{lemma4.19InverseAbschaetzung}\enter
Sei $\lbrace V_h\rbrace_{h\in I}$ eine Familie von affin-äquivalenten, formregulären Finiten-Elementen.\\
Dann gibt es eine konstante $c$, unabhängig von $h$, so, dass
\begin{align*}
|w_h|_{1,2,K}\leq c\cdot h^{-1}_K\cdot\Vert w_h\Vert_{1,2,K}\qquad\forall w\in V_h
\end{align*}
Global erhalten wir
\begin{align*}
|w_h|_{1,2,\Omega}\leq c\cdot h^{-1}_{\min}\cdot\Vert w_h\Vert_{0,2,\Omega}\qquad\forall w\in V_h\\
\text{wobei }h_{\min}:=\min\limits_{K\in\T_h} h_K
\end{align*}
\end{lemma}
\begin{proof}
\begin{align*}
