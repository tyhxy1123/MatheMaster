...
\begin{definition}[Finites Element]\enter %4.5
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item $V$ ist endlich-dimensionaler Funktionenraum von Funktionen mit Definitionsbereich $K$. $m:=\dim(V)<\infty$
		\item $\Sigma=\lbrace N_i\rbrace_{i\in\lbrace1,\ldots,m\rbrace}$ ist eine Menge von $m$ linearen Funktionalen\\ $N_1,\ldots,N_m:V\to\R^m$, welche mindestens für alle Funktionen in $V$ definiert sind.
		...
		\end{align*}
		Das bedeutet, es gibt Funktionen $v_j\in V$ mit
		\begin{align*}
...
\begin{definition}[Raum der finiten Elemente]\enter
	Sei $\T_h$ eine Triangulation von $\Omega$. Ein \textbf{Finite-Elemente-Raum} $V_h$ ist gegeben durch
	\begin{align*}
...
$:\gdw$
	\begin{enumerate}[label=(\roman*)]
		...
		\end{aligned}$
		\item Der Familienparameter h geht nach 0 $\iff$ die Dreiecke werden verfeinert, siehe \ref{AbbFamilienparameterKonvergenz}.
		\begin{figure}[!ht]
			\begin{center}
