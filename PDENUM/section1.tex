...
\begin{satz}[Spursatz]\label{satz1.7Spursatz}\enter
	...
	\end{align*}
	Da $\gamma_l$ stetig und linear ist, ist $\gamma_l$ auch beschränkt und man erhält die \textbf{Spurungleichung}
	\begin{align}\label{eqSpurungleichung}\tag{Spur}
		\exists c>0:\forall\varphi\in W^{k,p}(\Omega):\big\Vert\gamma_l\big\Vert_{L^p(\Omega)}\leq c\cdot\Vert\varphi\Vert_{W^{k,p}(\Omega)}
	\end{align}
\end{satz}
...
\begin{korollar}[Poincaré-Ungleichung]\label{korollarPoincareUngleichung}\enter %1.12
	...
	Setze
	\begin{align*}
		V:=\left\lbrace\varphi\in H^1(\Omega):\int\limits_\Omega \varphi(x)\d x=0\right\rbrace.
	\end{align*}
