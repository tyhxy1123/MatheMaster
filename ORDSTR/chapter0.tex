...
\begin{beispiel}
    ...
    \end{enumerate}
    \underline{Proposition:} Bildet $(S,\ast,\varepsilon)$ eine abelsche Gruppe, so ist eine Abbildung $\mu: \mathscr{A} \to S$ modular bzgl. $((\mathscr{A},\subseteq),(S,\ast,\varepsilon))$
    genau dann, wenn $\Delta^\mu: R \to S, (A,B) \mapsto (\mu A)^{-1}\ast \mu B$ modular bzgl. $(\G(\mathscr{A},R),(S,\ast,\varepsilon))$ ist.\\
    \underline{Begründung:} Dies folgt, da, falls $\mu$ modular, für $(A,B) \in R$ stets gilt 
    \begin{align*}
    ...
    \end{align*}
    \underline{Proposition:} Sei $\P = (P, R)$ Präordnung und sei $\mathcal{S} = (S,\ast,\varepsilon)$ Gruppe.\\
    Dann gilt für jede Abbildung $\mu: P \to S$ und $\Delta^\mu : R \to S, (a,b) \mapsto (\mu a)^{-1} \ast \mu b$,
    dass $(\G\P, \mathcal{S}, \Delta^\mu)$ Measurement Setup ist.\\
    \textit{Beweis.} Müssen zeigen, dass $\Delta^\mu$ funktoriell, also
    \begin{itemize}
    ...
    \end{itemize}
    \underline{Ergebins:} $(\G(\mathscr{A},R),(S,\ast,\varepsilon),\mu\circ\Delta = \Delta^\mu)$ ist Measurement Setup.\nl
    \underline{Proposition:} Bilde $\P = (?, \leq)$ einen Verband (d.h. $p \vee q$ ist die kleinste obere Schranke von $\{p,q\}$ in $\P$ und
    $p\wedge q$ ist die größte untere Schranke von $\{p,q\}$ in $\P$.) und $\mathcal{S} = (S,\ast,\varepsilon)$ eine abelsche Gruppe, dann sind
    für jede Abbildung $\mu: P \to S$ äquivalent:
    \begin{enumerate}[label=(\alph*)]
        \item $\mu$ ist modular bezüglich $(\P,\mathcal{S})$
        \item $\Delta^\mu: \leq_\P \to S, (p,q) \mapsto (\mu p)^{-1} \ast \mu q$ ist modular bezüglich $(\G\P,\mathcal{S})$.
    \end{enumerate}
\end{beispiel}
