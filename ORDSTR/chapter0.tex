...
\begin{definition}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        %\item \textbf{linear / total}, falls $pRq\vee qRp$. %gehört hier nicht wirklich hin
    \end{enumerate}
...
%	Dann ist die Multimenge $\mathcal{M}$ per \undefine{Dominanzordnung} geordnet.
%\end{lemma}
...
\begin{beispiel}
	...
	% Folgendes habe ich mal rausgenommen, da Lemma 1.1.9 dies besser formuliert
	%Sei $(S,\leq)$ lineare Ordnung (d.h. reflexiv, transitiv, antisymmetrisch und linear). 
	%Dann ist $(S,\leq)^\Omega:=(S^\Omega,\leq)$ Ordnung via
	%\begin{align*}
	%	\alpha\leq\beta:\Longleftrightarrow\forall x\in\Omega:\alpha x\leq\beta x
...
\textbf{Frage:} Seien $(\Omega,\leq)$ und $(S,\leq)$ lineare Ordnungen. 
...
\begin{beispiel}
	...
	$(\Z,\leq)$ ist \betone{keine} Wohlordnung.
\end{beispiel}
...
%	\end{align*}
%	Setze dann
%	$\alpha\sqsubseteq\beta:\gdw\Omega(\beta>\alpha)=\emptyset$ oder $\alpha=\beta$ oder $\Omega(\alpha<\beta)$
%	und $\Omega(\beta>\alpha)$ sind nichtleer und das kleinste Element von $\big(\Omega(\alpha<\beta),\leq\big)$ 
%	ist kleiner als das kleinste Element von $\big(\Omega(\beta<\alpha),\leq\big)$.
%\end{satz}

\begin{satz}\label{satzLexiko}
...
\begin{definition}
	...
	\begin{itemize}
		\item \define{ACC / noethersch} "Jede aufsteigende Kette terminiert"
		\begin{align*}
