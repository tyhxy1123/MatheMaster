...
We introduce the $n_{i} \times s n_{j}$ block matrices $A_{ij}$ and vectors $x_{i}, b_{i}$ as 
\[
...
.\] 
The matrix $V_{i}W_{i}^{T}$ is a projector from $\R^{n}$to a subspace $\Kappa_{i} \subset \R^{n}$ spanned by the columns of $V_{i}$. We call $W_{i}^{T}$ a \underline{restriction operator} and $V_{i}$ a \underline{prolongation operator}.
