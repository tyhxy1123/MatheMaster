...
\end{enumerate}
		...
		\begin{itemize}
			\item "$\Longrightarrow$" 
				...
				Assume that $x^{k}\rightarrow A^{-1}b$converges. Let $\lambda $ be the eigenvalue of $G$ with  $\abs{\lambda }  = \rho(G)$ and $x$ be the associated eigenvector.
...
\begin{figure}[H]
	...
	\end{center}
	\caption{Eigenwerte liegen genau gegenüber voneinander}
