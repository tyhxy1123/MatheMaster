...
\section{Relation between fine/coarse matrix $A^{h} / A^{H}$}%
...
\[
\Omega \approx \Omega _{h} = \bigcup_{i=1}^{m}T_{i} \qquad \tau_{h} := \left\{ T_{i} \right\} 
.\] 
Here $\tau_{h}$ is called conforming, if facets of a element $T_{i}$ are also facets of other elements $T_{j}$.

The mesh size $h$ is defined by
\[
h = \max\limits_{T \in\tau_{h}} \text{diam } T
.\] 
(Where diam is the length of the longest edge of an element $T_{i}$.)

An example of a finite-dimensional subspace is
\[
	V_{h}^{(0)} = \left\{ \phi \in C(\Omega _{h}) | \phi|_{T} \in \P_{p} \text{ for } T \in \tau_{h} \right\} \qquad (+ \text{ BC }, \phi|_{\partial \Omega _{h}}=0)
,\] 
...
\[
	a_{h}(\phi _{i}, \phi _{j}) = \sum_{T \in \tau_{h}}^{}{a_{T}(\phi _{i}, \phi _{j})}
,\] 
with
\[
...
\[
	A = \sum_{T \in \tau_{h}}^{}{A^{(T)}}= \sum_{T \in \tau_{h}}^{}{P\cdot A_{T} \cdot R}
,\] 
...
\begin{enumerate}[label=\alph{enumi})]
	...
	\item Adjacency graph of $A$
		...
		\[
		a_{i,j}\neq 0 \qquad \text{if } \exists T \in \tau_{h}\colon x_{i}\in T \text{ and } x_{j} \in T
		.\] 
