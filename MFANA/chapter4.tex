...
\begin{proof}
	Für alle $x\in X$ ist die Funktion $t\mapsto S_t x$ stetig auf $\R_+$ (Eigenschaft (iii)) und insbesondere auf dem Intervall $[0,1]$. 
...
\begin{definition}[Erzeuger]\enter
	...
	\begin{align*}
		A&:=\set[\bigg]{(u,f)\in X\times X:f=\lim\limits_{t\to0^+}\frac{S_t u-\overbrace{u}^{=S_0(u)}}{t}}\mit\\
...
\begin{theorem}[Exponentialformel]\label{theoremExponentialformel}\enter
	...
	Dann gilt
	\begin{align}\label{eqExpFormel}\tag{Exp}
		S_t u_0
		=\limn\left(I+\frac{t}{n}\cdot A\right)^{-n}u_0
		=\limn\left(J_{\frac{t}{n}}^A\right)^n u_0			
		\qquad\forall u_0\in D:=\overline{\dom(A)},\forall t\in\R_+
	\end{align}
	wobei $S$ die von $A$ "erzeugte" Halbgruppe ist (wie oben) und
	\begin{align*}
		\big(I+h\cdot A\big)^{-n}
		=\left((I+h\cdot A)^{-1}\right)^n	
		=J_h^n
	\end{align*}
\end{theorem}
