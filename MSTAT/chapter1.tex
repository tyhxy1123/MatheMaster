...
\begin{satz}[Teilfolgenprinzip für schwache Konvergenz]\label{satz4.11}\enter
\begin{enumerate}[label=(\arabic*)]
\item $\begin{aligned}
...
\section{Verteilungskonvergenz in $\R^d$}
...
\left\langle t,\sum\limits_{i=1}^n X_{n,k}\right\rangle\stackrel{\L}{\longrightarrow}\langle t,N\rangle\text{ in }\R\qquad\forall t\in\R^d
\end{align}
...
Y_{n,k}
&=\sum\limits_{k=1}^n t_j\cdot X_{n,k}^{(j)}
\end{align*}
