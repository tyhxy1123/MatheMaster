...
\text{denn }\xi^{-1}\big((-\infty,\varepsilon)\big)\in\A
\end{align*}
\end{definition}
...
=\P\left(\underbrace{\liminf\limits_{n\to\infty}\big(A_n(\varepsilon)\big)}_{\stackeq{\text{Def}}\bigcup\limits_{m\in\N}\bigcap\limits_{n\geq m}\big(A_n(\varepsilon)\big)^C=\bigcup\limits_{m\in\N}\bigcap\limits_{n\geq m}\big\lbrace d(X_n,X)\leq\varepsilon\big\rbrace}\right)\forall\varepsilon>0\\
&\stackrel{\text{abzählbarer Durchschnitt von Eins-Mengen}}{\implies}
\P\left(\underbrace{\bigcap\limits_{0<\varepsilon\in\Q}\bigcup\limits_{n\geq m}\big\lbrace d(X_n,X)\leq\varepsilon\big\rbrace}_{\lbrace X_n\to X\rbrace=\lbrace d(X_n,X)\to0\rbrace}\right)=1
\end{align*}
\end{proof}
...
\begin{align*}
X_n\stackrel{\P,~n\to\infty}{\longrightarrow}X:\Longleftrightarrow\forall\varepsilon>0:
\P\left(\left\lbrace d((X_n,X)>\varepsilon\right\rbrace\right)
\stackrel{n\to\infty}{\longrightarrow}
0
\end{align*}
\end{definition}

\begin{satz}\label{Satz3.12} %3.12
\begin{align*}
X_n
\stackrel{n\to\infty}{\longrightarrow}
X\quad\P\text{-fast sicher }
\implies X_n
\stackrel{\P,~n\to\infty}{\longrightarrow}
X
\end{align*}
\end{satz}
\begin{proof}
\begin{align*}
\forall\varepsilon>0:0\geq\P\big(d(X_n,X)>\varepsilon\big)
\leq\P\left(\sup\limits_{m\geq n}d(X_m,X)>\varepsilon\right)
\stackrel{\P,~n\to\infty}{\longrightarrow} 0
\end{align*}
...
X\text{ fast sicher}
\end{align*}
Zu $(X_{n'})$ existiert (wegen $X_n\stackrel{\P,~n\to\infty}{\longrightarrow} X'$ und \ref{satz3.3}) eine Teilfolge $(X_{n''})\subseteq(X_{n'})\mit:$
\begin{align*}
X_{n''}
