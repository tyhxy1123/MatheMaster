...
d_2\Big((x,x'),(y,y')\Big)&:=\max \left\lbrace d(x,y),d'(x',y')\right\rbrace\qquad\forall(x,x'),(y,y')\in \S\times\S'\\
d_3\Big((x,x'),(y,y')\Big)&:=d(x,y)+d'(x',y')\qquad\forall(x,x'),(y,y')\in \S\times\S'
\end{align*}
...
&=
\sigma\Big(\big\lbrace \overbrace{G\times G'}^{=(G\times S)\cap(S\times G')}:G,G\in\mathcal{G}\big\rbrace\Big)\\
&\stackeq{\text{($\ast$)}}
