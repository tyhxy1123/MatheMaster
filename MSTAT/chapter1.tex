...
\begin{align}\label{eqUnder8.2_1}\tag{1}
	...
	&=\left\lbrace\begin{array}{cll}
		...
		(n-k)\cdot\theta_n\cdot|\mu-\nu|, &\falls\tau_n<k\leq n &\text{monoton fallend}
	\end{array}\right.\\
	&\implies\tau_n=\arg\max\limits_{0\leq k\leq n}\big|\E[S_k]\big|\nonumber
\end{align}
%TODO Hier könnte man eine Skizze einfügen
...
\begin{align*}
	\hat{\tau}_n:=\arg\max\limits_{0\leq k\leq n}\big|S_k\big|
\end{align*}
...
\begin{align*}
	\hat{\theta}_n&:=\arg\max\limits_{0\leq t\leq 1}\big|M_n(t)\big|\\
...
\begin{align*}
	\theta=\arg\max\limits_{0\leq t\leq 1}\big|M(t)\big|
\end{align*}
...
\begin{align*}
	...
	\implies
	\P\Big(\big\Vert M_n-\overline{M}_n\big\Vert\geq\varepsilon\Big)
	&\leq\bigg(\max\limits_{0\leq k\leq n}\underbrace{\big|T_k\big|^p}_{\text{Submartingal}}\geq\underbrace{2^{-p}\cdot n^p\cdot\varepsilon^p}_{=:y>0}\bigg)\\
