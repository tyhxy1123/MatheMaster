...
\begin{beispiel}
    ...
    und maximalem Ideal
    $$ m_p := m_{v_p} = pK[T]_{(p)}.$$
    Der Restklassenkörper ist dann
    $$ \kappa_p := \kappa_{\mathcal{O}_{v_p}} = \sfrac{K[T]}{(p)}, \text{ denn}$$
    $$\begin{tikzcd}
        K[T] \arrow[d, hook, "g"] \arrow[dr, "\phi"]\\
        K[T]_{(p)} \arrow[r, two heads, "h"] & K[T]_{(p)}/pK[T]_{(p)}
    \end{tikzcd}$$
    \begin{align*}
        \ker \phi = (p) \implies K[T]/(p) \cong \im \phi = \im h \circ g = K[T]_{(p)}/pK[T]_{(p)}
    \end{align*}
