...
\newcommand{\scaProd}[2]{\left\langle#1, #2\right\rangle} % scalar product, could also use (, )
\newcommand{\dualPair}[2]{#1\left(#2\right)}   % dual pairing of X' and X for a Banach (in our case Hilbert) space and its dual space, in original ⟨, ⟩ is used
...
\newcommand{\Laplace}{\Delta}   % Laplace operator ist nicht ein griechisches Delta und sollte im Code auch anders heißen. Wenn man möchte, könnte man auch ein leicht anderes Symbol nutzen.
\newcommand{\Lap}{\Laplace}   % Abkürzung
\newcommand{\setDef}[2]{\left\{#1 \,\middle:\, #2\right\}}            % a typical set definition: #1 is the thing in the set #2 the condition on the variables {#1 | #2}
